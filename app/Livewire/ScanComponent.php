...
class ScanComponent extends Component
{
    ...
    public function scan(string $barcode)
    {
        if (is_null($this->currentLiveCoords)) {
            ...
            // Safety check: jika shift_id masih null, paksa ke 1
            $this->shift_id = 1;
        }
    ...
    public function createAttendance(Barcode $barcode)
    {
        ...
        // Memastikan Shift ID 1 terpilih
        $targetShiftId = $this->shift_id ?? 1;
        
        /** @var Shift */
        $shift = Shift::find($targetShiftId);
        
        // Logika Status: Jika jam sekarang lebih besar dari jam mulai shift -> Late
        // Shift 1 diset 08:00, jadi absen 08:01 otomatis 'late'
    ...
    public function mount()
    {
        ...
        // MEMAKSA sistem untuk selalu memilih Shift ID 1.
        // Logika "ExtendedCarbon" yang menebak shift dihapus total.
        // ---------------------------------------------------------
        $this->shift_id = 1; 
